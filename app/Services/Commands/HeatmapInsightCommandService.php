...
{
    /**
     * Tạo insight TYPE_REBOUND (tối ưu chỉ giữ các trường cần thiết)
     */
    ...
    /**
     * Xử lý toàn bộ heatmap và tạo insights (theo thuật toán tối ưu mới)
     */
    public function process(array $heatmap, $anchor_date, $streak_min = 4, $day_stops_max = 6, $running_max = 3): void
    {
        ...
        echo "[DEBUG] ids_by_date: ".json_encode($ids_by_date)."\n";
        ...
        foreach ($ids_by_date as $date => $ids) {
            foreach ($ids as $id) {
                ...
                for ($i = 0; $i <= $start_idx; $i++) { // từ anchor_date đến ngày xuất hiện
                ...
                $streaks = array_map(function($item) {
                    return $item['streak'];
                }, $timeline);
                ...
                for ($j = count($streaks) - 1; $j >= 0; $j--) { // từ ngày cũ nhất lên anchor
                    $streak = $streaks[$j];

                    // Tìm streak đầu tiên >= streak_min
                    if ($streak >= $streak_min && $streak_length === null) {
                        $streak_length = $streak;
                        continue;
                    }

                    // Đã có streak_length, đếm stop_days, cờ dấu hiệu
                    if ($streak_length !== null) {
                        if ($streak == 0 && !$flag_stop_days) {
                            if ($stop_days < $day_stops_max) {
                                $stop_days++;
                            } else {
                                break; // Vượt quá số ngày dừng tối đa
                            }
                        }
                        //Lần đầu tiên gặp streak == 1 quyết định phân hóa luôn
                            ...
                            $step_2 = null;
                            $step_3 = null;
                        } else if($running_max_value > 0 && $flag_stop_days){  //các lần sau đó
                            $running_max_value++;
                            ...
                            echo "[DEBUG] Loại bỏ do có streak >= 4 xen kẽ, id: $id, date: {$timeline[$j]['date']}\n";
                            break;
                        }
                        // Chỉ tạo insight khi duyệt đến anchor_date và thỏa điều kiện
